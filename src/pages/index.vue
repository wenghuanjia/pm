  ...
  <div class="main">
    ...
    <div class="lists">
      <div class="list clear" v-for="(item, index) in lists" :key="index">
        ...
        <p class="fl" v-else-if="item.status == 1">
          {{ item.name }}已完成填报，请点击
          <router-link :to="{path: '/staff', query: {'search': item.id}}">进入分配</router-link>
        </p>
        <p class="fl" v-else-if="item.status == 3">
          {{ item.name }}已完成填报，请点击
          <router-link :to="{path: '/company', query: {'search': item.id}}">进入审核</router-link>
        </p>
        <p class="fl" v-else>
          {{ item.name }}已开通填报，请点击
          <router-link :to="{path: '/company', query: {'search': item.id}}">进入填表</router-link>
        </p>
