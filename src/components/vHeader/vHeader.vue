<template>
  <header>
    ...
    <el-menu
      ...
      mode="horizontal"
      @select="handleSelect"
      background-color="#287DD4"
      text-color="#fff"
      active-text-color="#fff"
    >
      <el-menu-item index="/index">
        <i class="el-icon-date el"></i>
        <span style="vertical-align: middle;">首页</span>
      </el-menu-item>
      <el-menu-item index="/company">
        <i class="el-icon-edit-outline el"></i>
        <span style="vertical-align: middle;">企业管理</span>
      </el-menu-item>
      <el-submenu index="/pm">
        <template slot="title">
          <i class="el-icon-edit el"></i>
        ...
        <el-menu-item
          ...
          :key="index"
          @click="whileId(item.id)"
        >{{ item.name }}</el-menu-item>
      </el-submenu>
      <el-menu-item index="/staff" v-if="userInfo.name == '管理员'">
        <i class="el-icon-menu el"></i>
        <span style="vertical-align: middle;">人员管理</span>
      </el-menu-item>
    </el-menu>
    <section class="crumbs-wrapper clear" v-if="!getaddpm">
      <div class="crumbs_title fl">
        {{ crumbs_title }}
        <span v-if="smallTitle" style="font-size: 12px;">{{ smallTitle }}</span>
      </div>
      <div class="fr clear">
...
export default {
  ...
  methods: {
    ...
    confirm() {
      this.dialogFormVisible = false;
      if (this.form.name == "" || this.year == "")
        return this.$message("请输入企业名称");
      this.typeArr.forEach(item => {
        console.log(item);
        console.log(this.year);
        addProName({
          name: this.form.name,
          project_type_id: item,
          year: this.year
        }).then(res => {
